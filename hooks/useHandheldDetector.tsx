...
const useHandheldDetector = (options?: { customBreakpoint?: number }): boolean => {
  const [isHandheld, setIsHandheld] = useState<boolean>(false);

  useEffect(() => {
    const mediaQuery = window.matchMedia(`(max-width: ${options?.customBreakpoint ? options.customBreakpoint : "1023"}px)`);
