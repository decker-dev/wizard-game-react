import { HealthPack, GameState, Vector2 } from '@/types/game'
import {
  HEALTH_PACK_SIZE,
...
export const spawnHealthPacksForWave = (
  ...
  const maxPacksThisWave = Math.floor(Math.random() * HEALTH_PACK_MAX_PER_WAVE) + 1
  ...
  for (let i = 0; i < packsToSpawn; i++) {
    ...
    if (position) {
      const healthPack = createHealthPack(position.x, position.y, healthPackSprite)
...
const findValidHealthPackPosition = (gameState: GameState): Vector2 | null => {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    const x = Math.random() * (MAP_WIDTH - HEALTH_PACK_SIZE * 2) + HEALTH_PACK_SIZE
    const y = Math.random() * (MAP_HEIGHT - HEALTH_PACK_SIZE * 2) + HEALTH_PACK_SIZE
...
export const collectHealthPack = (
  ...
  if (checkAABBCollision(playerRect, healthPackRect)) {
    ...
    const newHealth = Math.min(player.health + healthPack.healAmount, player.maxHealth)
