import { Vector2, Creature, Obstacle } from '@/types/game'
import { normalize, distance, magnitude } from '@/utils/math'
...
export class AIBehaviors {
  ...
  static seekWithPathfinding(target: Vector2, creature: Creature, obstacles: Obstacle[], maxForce: number = 0.8): SteeringForce {
    const now = Date.now()
    const pathUpdateInterval = 500 // Actualizar path cada 500ms

    // Verificar si necesitamos actualizar el path
    const needsNewPath = !creature.currentPath ||
    ...
    if (currentTarget) {
      ...
      const currentDirection = normalize({
        x: creature.position.x - (creature.lastPosition?.x || creature.position.x),
        y: creature.position.y - (creature.lastPosition?.y || creature.position.y)
      })
  ...
  static seek(target: Vector2, creature: Creature, maxForce: number = 0.1): SteeringForce {
    const desired = normalize({
  ...
  static flee(target: Vector2, creature: Creature, maxForce: number = 0.15): SteeringForce {
    const desired = normalize({
  ...
  static separate(neighbors: Creature[], creature: Creature, separationRadius: number = 50): SteeringForce {
  ...
  static avoidObstacles(obstacles: Obstacle[], creature: Creature, avoidanceRadius: number = 60): SteeringForce {
    ...
    for (const obstacle of obstacles) {
      ...
      if ((dist < avoidanceRadius || lookAheadDist < avoidanceRadius) && dist < closestDistance) {
    ...
    if (closestObstacle) {
      ...
      const avoidanceStrength = (avoidanceRadius - closestDistance) / avoidanceRadius
  ...
  static circle(target: Vector2, creature: Creature, circleRadius: number = 250): SteeringForce {
...
export class CreatureAI {
  static updateNormalCreature(
    ...
    const seekForce = AIBehaviors.seekWithPathfinding(target, creature, obstacles, 0.8)
  ...
  static updateCasterCreature(
    ...
    if (distanceToTarget < 200) {
      ...
      if (pathfinder.hasLineOfSight(creature.position, target, obstacles)) {
        ...
        const seekForce = AIBehaviors.seekWithPathfinding(fleeTarget, creature, obstacles, 0.8)
        forces.push({ x: seekForce.x * 1.2, y: seekForce.y * 1.2 })
      }
    } else if (distanceToTarget > 400) {
      // Muy lejos - acercarse usando pathfinding
      const seekForce = AIBehaviors.seekWithPathfinding(target, creature, obstacles, 0.6)
