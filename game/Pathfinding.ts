import { Vector2, Obstacle } from '@/types/game'
...
export class Pathfinding {
  ...
  constructor(mapWidth: number, mapHeight: number, nodeSize: number = 20) {
  ...
  findPath(startPos: Vector2, targetPos: Vector2): Vector2[] {
    ...
    if (startNode.x < 0 || startNode.x >= this.gridWidth ||
      startNode.y < 0 || startNode.y >= this.gridHeight ||
    ...
    while (openSet.length > 0) {
      ...
      if (currentNode.x === targetGridNode.x && currentNode.y === targetGridNode.y) {
  ...
  findNearestWalkableNode(targetNode: { x: number, y: number }): PathNode | null {
    const searchRadius = 5
    let nearestNode: PathNode | null = null
    let nearestDistance = Infinity

    for (let x = Math.max(0, targetNode.x - searchRadius);
      x < Math.min(this.gridWidth, targetNode.x + searchRadius); x++) {
      for (let y = Math.max(0, targetNode.y - searchRadius);
        y < Math.min(this.gridHeight, targetNode.y + searchRadius); y++) {
  ...
  simplifyPath(path: Vector2[]): Vector2[] {
    ...
    for (let i = 1; i < path.length - 1; i++) {
      ...
      const cross = (next.x - prev.x) * (current.y - prev.y) - (next.y - prev.y) * (current.x - prev.x)
